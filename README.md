...
### 대규모 검증 — 계좌 10만 · 거래 1천만

로컬 계좌 3개로는 재현 못 했던 질문(계좌당 매칭률이 낮아지면 거래내역 조회 실행계획이 바뀌는가, `count(*)`가 병목이 되는가)을 실제 규모로 확인했습니다. 데이터를 1천만 건까지 채운 뒤에도 조회·이체 API 전부 기준(200ms) 대비 15배 이상 여유였고 실패율은 0%였습니다.

| 엔드포인트 | p95 | 처리량 | 실패율 |
|---|---|---|---|
| 거래내역 조회 | 11.67ms | 99.55 req/s | 0.00% |
| 잔액 조회 | 7.55ms | 99.69 req/s | 0.00% |
| 계좌 상세 | 7.91ms | 99.73 req/s | 0.00% |
| 고객별 계좌 목록 | 7.69ms | 99.65 req/s | 0.00% |
| 이체 | 12.89ms | 99.66 req/s | 0.00% |

`EXPLAIN`으로 W6에 추가해둔 복합 인덱스가 이 시점에 실제로 `BitmapOr`로 쓰이는 것까지 확인했습니다 — 코드 변경 없이 옵티마이저 판단만으로 값을 했습니다. 시드·측정 절차와 EXPLAIN 전문은 [대규모 성능 측정 devlog](docs/devlog/2026-09-10_대규모1천만건성능측정.md)에 있습니다.
...
코드·ADR·devlog 근거를 바탕으로 우선순위별 한계와 예상 공격 질문을 스스로 정리해뒀습니다. 그중 즉시 답변이 필요한 항목 세 가지입니다.
...
6. [알려진 한계와 개선과제](docs/11_J-Bank_알려진한계와개선과제.md) — 구현 후 스스로 정리한 한계와 예상 질문
