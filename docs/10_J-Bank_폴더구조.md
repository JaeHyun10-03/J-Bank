...
멀티모듈의 이점은 컴파일 시점에 의존을 막아준다는 것이다. 이점은 있지만, 그 대가로 W1에 모듈 경계를 확정해야 한다.
...
### 3.4 단일 트랜잭션 경계를 유지한다

상품 가입은 계약 생성·계좌 출금·원장 기록을 같은 DB 트랜잭션에서 처리한다. 만기 배치도 이자 입금과 계약 만기 전환을 함께 커밋한다. 패키지 경계는 유지하고 네트워크 호출 없이 서비스와 리포지토리로 협력한다.
...
| product | account, transfer | 계좌 소유권·상태를 확인하고 WithdrawalService로 초기 납입금을 출금한다 |
