...
/**
 * 피그마 로그인 화면(Auth11)은 이 기기가 이미 사용자를 기억한다는 전제로 PIN
 * 넘패드만 보여준다. 최초 접속 등 저장된 로그인ID가 없을 때 이 화면(다른 방법으로
 * 로그인)으로 온다 — 아이디/비밀번호 폼 자체는 피그마에 대응 화면이 없다.
 */
export default function LoginIdPage() {
  ...
  async function onSubmit(values: LoginForm) {
    ...
    try {
      ...
      window.localStorage.setItem(LAST_LOGIN_ID_KEY, values.loginId);
      router.push("/");
    } catch (error) {
