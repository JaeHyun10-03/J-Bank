...
/**
 * 배치 잡 단일 실행을 보장하는 리스너(구현계획 W7). K8s CronJob의 concurrencyPolicy: Forbid가 평상시 중복 실행을 막아주지만, 인스턴스를 여러
 * 개로 늘리는 시점부터는 수동 재실행(kubectl create job --from=cronjob) 같은 경로로 같은 잡이 동시에 시작될 여지가 생긴다(ADR 0003). 잡
 * 시작 전 락을 못 잡으면 스텝을 하나도 실행하지 않고 그대로 실패시킨다 — beforeJob에서 예외를 던지면 AbstractJob#execute가 스텝 실행 전에 잡을
...
public class SingleInstanceJobExecutionListener implements JobExecutionListener {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(SingleInstanceJobExecutionListener.class);
