...
public interface ProductContractRepository extends JpaRepository<ProductContract, Long> {
  ...
  // 이자 계산·만기 처리 배치가 기준일까지 만기가 도래한 계약을 페이지 단위로 읽는다(구현계획 W5).
  // exclusiveUpperBound는 기준일 다음날 자정이라 기준일 당일 만기까지 포함한다.
  // 처리 중 ACTIVE → MATURED 전환으로 페이지가 줄어 누락되지 않도록 상태로 필터링하지 않는다.
  Page<ProductContract> findByMaturityAtLessThan(
      OffsetDateTime exclusiveUpperBound, Pageable pageable);
}
